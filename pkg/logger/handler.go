...
// receiver's existing group. The keys of all subsequent attributes, whether
// added by With on in a Record, should be qualified by the sequence of
// group names.
func (h *logHandler) WithGroup(name string) slog.Handler {
	return &logHandler{handler: h.handler.WithGroup(name), events: h.events}
}

// Handle looks to use if an event function needs to be executed for a given
